...
export default function Wishlist() {
        ...
        const initialize = async () => {
            ...
            try {
                ...
                try {
                    ...
                    if (currentUser && currentUser.family_id === memberData.family_id) {
                        isMemberOfFamily = true;
                    }
                } catch (e) { /* Niet ingelogd, wat prima is */ }

                setIsFamilyMember(isMemberOfFamily);

                if (isMemberOfFamily || !memberData.wishlist_password) {
    ...
    const loadItems = async () => {
        if (!memberId) return;
        const wishlistItems = await WishlistItem.filter({ family_member_id: memberId });
        setItems(wishlistItems.sort((a, b) => (a.status > b.status) ? 1 : -1));
    };

    const handlePasswordSubmit = (e) => {
        e.preventDefault();
        if (password === member.wishlist_password) {
            setAccessState('visible');
            loadItems();
            setErrorMessage('');
        } else {
            ...
            <div className="min-h-screen bg-gray-50 p-4 sm:p-8">
                <div className="max-w-3xl mx-auto">
                    <header className="text-center mb-8">
                        ...
                        <h1 className="text-4xl font-bold text-gray-800">{member?.name}'s verlanglijst</h1>
                        <p className="text-gray-500 mt-1">Hier zijn wat cadeau-ideeën om hun dag speciaal te maken!</p>
                    </header>
            ...
            <div className="min-h-screen bg-gradient-to-br from-purple-50 to-pink-50 p-6">
                <div className="max-w-4xl mx-auto">
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
                        <div>
                            <div className="w-16 h-16 rounded-full mb-4 flex items-center justify-center text-white text-2xl font-bold shadow-lg" style={{ backgroundColor: member?.color }}>
                                {member?.name.charAt(0).toUpperCase()}
                            </div>
                            <h1 className="text-3xl font-bold text-gray-900 flex items-center gap-3">
                                <Gift className="w-8 h-8 text-pink-500" />
                                Verlanglijst voor {member?.name}
                            </h1>
                            {member?.dob && (
    ...
    // Fallback if state is not recognized (should not happen)
    return null;
}
