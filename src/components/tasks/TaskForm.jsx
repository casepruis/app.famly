...
import { Calendar as CalendarIcon, X, Users, Repeat, Loader2 } from "lucide-react";
...
export default function TaskForm({
  ...
  onSave,
  familyMembers,
  /** NEW: busy flags for UX feedback */
  busy = false,
  inferring = false,
  saving = false,
}) {
  ...
  useEffect(() => {
    if (!isOpen) return;
    if (task) {
      // Merge existing task values with sensible defaults
      setCurrentTask({
        title: task.title ?? "",
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (busy) return; // prevent double-submit while LLM/saving
    onSave(currentTask);
  };
  ...
  const getSelectedMemberNames = () => {
    if (!currentTask.assigned_to?.length) return t("assignToDefault");
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-lg max-h-[85vh] flex flex-col">
        <DialogHeader>
          ...
          {/* NEW: inline status row under title */}
          {busy && (
            <div className="mt-2 text-xs text-gray-500 flex items-center gap-2">
              <Loader2 className="h-4 w-4 animate-spin" />
              {inferring
                ? (t("task.inferencingDueDate") || "Analyzing task title for a due date…")
        ...
        <form
          ...
          className="flex-1 overflow-y-auto pr-2 space-y-4 p-1"
          aria-busy={busy}
        >
          <Input
                ...
                {currentTask.assigned_to.map((memberId) => {
                  ...
                  return member ? (
                    ...
                    >
                      ...
                      <button
                        type="button"
                        onClick={() => !busy && removeMember(memberId)}
                        className="ml-1 hover:bg-gray-200 rounded-full p-0.5 disabled:opacity-50"
                        disabled={busy}
                      >
                        <X className="w-3 h-3" />
                      </button>
          ...
          <div className="space-y-3">
            <div className="flex items-center space-x-2">
              <Checkbox
                ...
                onCheckedChange={(checked) =>
                  setCurrentTask({ ...currentTask, is_recurring: !!checked })
                }
                disabled={busy}
              />
            ...
            {currentTask.is_recurring && (
              <Select
                ...
                onValueChange={(value) =>
                  setCurrentTask({ ...currentTask, recurrence_pattern: value })
                }
                disabled={busy}
              >
                <SelectTrigger>
          ...
          >
            {busy ? (
              <span className="inline-flex items-center gap-2">
                <Loader2 className="h-4 w-4 animate-spin" />
                {inferring ? (t("analyzing") || "Analyzing…") : (t("saving") || "Saving…")}
              </span>
            ) : task ? (
              t("updateTask")
            ) : (
