...
export default function InlineWishlistReview({ item, itemIndex, familyMembers, onWishlistUpdate }) {
  ...
  return (
    <div className="flex items-center gap-3 p-2 bg-purple-50/80 border border-purple-200/90 rounded-lg text-xs w-full shadow-sm">
      <Checkbox
      ...
      <Input
        ...
        className="flex-1 h-7 text-xs border-purple-300 bg-white"
        ...
        {familyMembers.map(member => (
          <div key={member.id} className="flex items-center gap-1.5">
            <Checkbox
              ...
              className="h-4 w-4"
            />
            <label htmlFor={`wishlist-${itemIndex}-member-${member.id}`} className="text-xs font-medium text-purple-800 cursor-pointer">{member.name}</label>
          </div>
        ))}
      </div>
    </div>
